                                ...
                                <table cell class="table table-striped table-bordered table-hover" id="myTable">
                                    ...
                                    <tbody>         
                                        <?php
                                            ...
                                            /*$conn = mysqli_connect('localhost', 'findpata_snippetc', 'Manusoftwares2946', 'findpata_sokoni') or die(mysqli_error($conn));*/
                                            $result = mysqli_query($conn,'select * from mychama_user_loans where chama = "'.$session.'" and status="new"') or die(mysqli_error($conn));
                                            while ($row=mysqli_fetch_assoc($result)) {
                                           ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-success btn-xs" href="group_loans.php?id=<?php echo $row['id']; ?>&st=acc" onclick="return confirm('Are you sure to approve this loan?');">Approve</a>
                                                    <a class="btn btn-danger btn-xs" href="group_loans.php?id=<?php echo $row['id']; ?>&st=rej" onclick="return confirm('Are you sure to reject this loan?');"><i class="fa fa-times"></i>Reject</a>
                                                <?php
                                                    if (isset($_GET['id'])) {
                                                        $delete_id = $_GET['id'];
                                                        $get_id = $_GET['st'];
                                                        if ($get_id == "acc") {
                                                        $mydate = date('Y-m-d', strtotime($row['date']. ' + 90 days'));
                                                        mysqli_query($conn,'update mychama_user_loans set status="approved", return_date="'.$mydate.'" where id = "'.$delete_id.'"') or die(mysqli_error($conn));
                                                        echo "<script>window.alert('Loan approved successfully!');</script>";
                                                        echo '<META HTTP-EQUIV="refresh" content="0;URL=app_loans.php">';
                                                        }
                                                        elseif($get_id == "rej"){
                                                        mysqli_query($conn,'update mychama_user_loans set status="rejected", return_date="rejected" where id = "'.$delete_id.'"') or die(mysqli_error($conn));
                                                        echo "<script>window.alert('Loan rejected successfully!');</script>";
                                                        echo '<META HTTP-EQUIV="refresh" content="0;URL=rej_loans.php">';   
                                                        }
                                                    }
                                                 ?></td>
                                            </tr>
