                    ...
                    <div class="portlet light">
                        <div class='row'>
                            <strong>Live search:</strong>
                                <input type="text" id="myInput" name="search" placeholder="Loan status" onkeyup="myFunction()" style="border: 1px solid #cccccc; border-radius: 4px;-webkit-box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075);box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075);-webkit-transition: border-color ease-in-out .15s, box-shadow ease-in-out .15s;transition: border-color ease-in-out .15s, box-shadow ease-in-out .15s; padding: 3px 8px; vertical-align: middle; background-color: #ffffff;"><br><br>
                            <div class="col-md-12">
                                ...
                                <table cell class="table table-striped table-bordered table-hover" id="myTable">
                                    ...
                                    <tbody>         
                                        <?php
                                            ...
                                            /*$conn = mysqli_connect('localhost', 'findpata_snippetc', 'Manusoftwares2946', 'findpata_sokoni') or die(mysqli_error($conn));*/
                                            $result = mysqli_query($conn,'select * from mychama_user_loans where chama = "'.$session.'" and status="approved"') or die(mysqli_error($conn));
                                            while ($row=mysqli_fetch_assoc($result)) {
                                           ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-success btn-xs" href="app_loans.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure this loan was paid back?');">Paid</a>
                                                <?php
                                                    if (isset($_GET['id'])) {
                                                        $delete_id = $_GET['id'];
                                                        mysqli_query($conn,'update mychama_user_loans set status="paid" where id = "'.$delete_id.'"') or die(mysqli_error($conn));
                                                        echo "<script>window.alert('Loan paid back successfully!');</script>";
                                                        echo '<META HTTP-EQUIV="refresh" content="0;URL=paid_loans.php">';
                                                    }
                                                 ?></td>
                                            </tr>
